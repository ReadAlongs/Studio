...
class TestDNAUtils(TestCase):
    ...
    def test_sort_and_join_dna_segments(self):
        ...
        assert sort_and_join_dna_segments(
            segments_from_pairs((1000, 1100), (1500, 2100))
        ) == segments_from_pairs((1000, 1100), (1500, 2100))
        assert sort_and_join_dna_segments(
            segments_from_pairs((1500, 2100), (1000, 1100))
        ) == segments_from_pairs((1000, 1100), (1500, 2100))
        assert sort_and_join_dna_segments(
            segments_from_pairs((2, 3), (11, 14), (23, 25), (1, 4), (12, 13), (24, 26))
        ) == segments_from_pairs((1, 4), (11, 14), (23, 26))

    def test_adjustment_calculation(self):
        """Try adjusting alignments of re-built audio"""
        assert calculate_adjustment(1000, [{"begin": 1000, "end": 1100}]) == 100
        assert calculate_adjustment(900, [{"begin": 1000, "end": 1100}]) == 0
        assert calculate_adjustment(1000, [{"begin": 1000, "end": 1100}]) == 100
        assert calculate_adjustment(900, [{"begin": 1000, "end": 1100}]) == 0
        assert calculate_adjustment(2000, [{"begin": 1000, "end": 1100}]) == 100
    ...
    def test_adjustment_correction(self):
        ...
        assert correct_adjustments(950, 1125, [{"begin": 1000, "end": 1100}]) == (
            950,
            1000,
        )
        assert correct_adjustments(975, 1150, [{"begin": 1000, "end": 1100}]) == (
            1100,
            1150,
        )
        # Function only accepts args in ms
        assert correct_adjustments(0.950, 1.125, [{"begin": 1000, "end": 1100}]) != (  # type: ignore[arg-type]
            950,
            1000,
        )
        assert correct_adjustments(0.975, 1.150, [{"begin": 1000, "end": 1100}]) != (  # type: ignore[arg-type]
            1100,
            1150,
        )
    ...
    def test_segment_intersection(self):
        ...
        assert segment_intersection(segments_from_pairs((1, 3)), []) == []
        assert segment_intersection([], segments_from_pairs((1, 3))) == []
        assert segment_intersection(
            segments_from_pairs(
    ...
    def test_dna_union(self):
        ...
        assert dna_union(
            1000, 2000, 3000, [{"begin": 1100, "end": 1200}]
        ) == segments_from_pairs((0, 1000), (1100, 1200), (2000, 3000))
        assert dna_union(
            None, 2000, 3000, [{"begin": 1100, "end": 1200}]
        ) == segments_from_pairs((1100, 1200), (2000, 3000))
        assert dna_union(
            1000, None, 3000, [{"begin": 1100, "end": 1200}]

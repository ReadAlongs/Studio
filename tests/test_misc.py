...
class TestMisc(BasicTestCase):
    ...
    def test_parse_time(self):
        ...
        for time_str, time_in_ms in (
            ...
            assert parse_time(time_str) == time_in_ms, f'error parsing "{time_str}"'
    ...
    <meta name="generator" content="@readalongs/studio (cli) %s"/>
        ...
        for i, s, lang in zip(
            ...
            assert (
                get_attrib_recursive(s, "lang") == lang
        ...
        for i, s, xml_lang in zip(
            ...
            assert (
                get_attrib_recursive(s, "xml:lang") == xml_lang
    ...
    def test_get_word_text(self):
        assert get_word_text(parse_xml("<w>basicword</w>")) == "basicword"
        assert get_word_text(parse_xml("<w><subw>subwcase</subw></w>")) == "subwcase"
        assert (
            get_word_text(parse_xml("<w><syl>syl1</syl><syl>syl2</syl></w>"))
            == "syl1syl2"
        )
        assert (
            get_word_text(parse_xml("<w>text<subw>sub</subw>tail</w>")) == "textsubtail"
        )
        assert get_word_text(parse_xml("<w><a>a<b>b</b>c</a>d</w>")) == "abcd"
    ...
    def test_load_xml(self):
        ...
        assert xml_text.encode(encoding="ascii") == etree.tostring(load_xml(foo_file))
    ...
    def test_load_xml_errors(self):
        ...
        with open(explode_file, "w") as f:
            f.write(xml_bomb)
        assert etree.tostring(load_xml(explode_file)) == b"<explode>&c;&c;</explode>"
